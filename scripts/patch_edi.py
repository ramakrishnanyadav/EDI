...
with open(BASE_DIR / ".env", "r") as f:
    env_content = f.read()
env_content = env_content.replace("ANTHROPIC_API_KEY=your_anthropic_api_key", "GEMINI_API_KEY=your_gemini_api_key")
with open(BASE_DIR / ".env", "w") as f:
    f.write(env_content)

with open(BASE_DIR / "requirements.txt", "r") as f:
    req_content = f.read()
req_content = req_content.replace("anthropic\n", "google-genai\n")
...
async def extract_decision(thread_id: str, url: str, raw_text: str) -> dict[str, Any]:
    ...
    if not api_key or api_key == "your_gemini_api_key":
        logger.warning("No valid GEMINI_API_KEY, skipping extraction.")
        return {}
...
async def query_edi(description: str, context: dict) -> dict:
    ...
    # Step 1: Extract problem slug and context from user query via Gemini
    ...
    # Step 7: Synthesize natural language answer with Gemini
...
print("Migration to Gemini complete.")
